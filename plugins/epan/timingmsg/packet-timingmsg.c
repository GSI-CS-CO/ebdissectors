#include "config.h"
#include "packet-timingmsg.h"
#include "evt_out.h"

#include <stdio.h>
#include <epan/packet.h>

proto_tree* addEbRecord(proto_tree *root _U_, tvbuff_t *tvb, gint *pOffs, const guint64 ts, const gint adrSelBit, const gint datSelBit, const char * name ) {
    ...
    while (wrcnt >= 8) {
        ...
        (void)wrTree;
        //wrTree = proto_tree_add_subtree(retTree, tvb, lOffs, wrcnt*alignment, ebrt_eb, NULL, "Tmsg");
        //check if address is correct lOffs+alignment-adrWidth, adrWidth,
        //proto_tree_add_item(wrTree, recf_hdr, tvb, lOffs+alignment-adrWidth, adrWidth, ENC_BIG_ENDIAN);
        lOffs += alignment;
        //const guint32 PMODE_NONE     = 0x0;
        //const guint32 PMODE_DEC      = 0x1;
        const guint32 PMODE_HEX      = 0x2;
        const guint32 PMODE_VERBOSE  = 0x4;
        const guint32 PMODE_UTC      = 0x8;
...
static int
dissect_timingmsg(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree _U_, void *data _U_)
{
    guint64 ts = pinfo->abs_ts.secs * 1000000000ULL + pinfo->abs_ts.nsecs;
    ...
    /*is this strictly necessary? it could be multicast in a test environment I suppose */
    //check preconditions for a timing message: it's broadcast dst MAC and IP, its not configspace, its no reads,  fifo mode and 32b 32b
...
void
proto_reg_handoff_timingmsg(void)
{
    static dissector_handle_t timingmsg_handle;

    timingmsg_handle = create_dissector_handle(dissect_timingmsg, proto_timingmsg);
    dissector_add_uint("udp.port", timingmsg_PORT, timingmsg_handle);
}



void
proto_register_timingmsg(void)
{
    static hf_register_info hf[] = {
         
            {&hf_eb_magic,
                { "Magic", "timingmsg.magic",
                FT_UINT16, BASE_HEX,
                NULL, 0x0,
                NULL, HFILL}
            },
            ...
            {&hf_eb_probeflag,
                { "Probe request", "timingmsg.ebhdr.probe",
                FT_BOOLEAN, 8,
            ...
            {&hf_eb_presponseflag,
                { "Probe response", "timingmsg.ebhdr.presponse",
                FT_BOOLEAN, 8,
            ...
            {&hf_eb_noreadflag,
                { "No reply", "timingmsg.ebflags.noread",
                FT_BOOLEAN, 8,
            ...
            {&hf_eb_version,
                { "Protocol Version", "timingmsg.ebflags.version",
                FT_UINT8, BASE_DEC,
                NULL, EB_HDR_VERSION,
                NULL, HFILL}
            },
            ...
            //in eb spec, but apparently not implemented
            {&hf_padding,
                { "Padding to 64b alignment", "timingmsg.padding",
                FT_UINT32, BASE_HEX,
                NULL, 0x0,
                NULL, HFILL}
            }      
...
/*
        static hf_register_info tmsgf[] = {
            ...
            {&tmsgf_id_fid,
                { "FID", "ebrec.msg.id.fid",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_FID,
                NULL, HFILL}
            },
            {&tmsgf_id_gid,
                { "GID", "ebrec.msg.id.gid",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_GID,
                NULL, HFILL}
            },
            {&tmsgf_id_evno,
                { "EvNO", "ebrec.msg.id.evno",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_EVNO,
                NULL, HFILL}
            },
            {&tmsgf_id_bin,
                { "Beam-in", "ebrec.msg.id.bin",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_BID,
                NULL, HFILL}
            },
            {&tmsgf_id_bs,
                { "BPC-Start", "ebrec.msg.id.bs",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_BS,
                NULL, HFILL}
            },
            {&tmsgf_id_sid,
                { "SID", "ebrec.msg.id.sid",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_SID,
                NULL, HFILL}
            },
            {&tmsgf_id_bpid,
                { "BPID", "ebrec.msg.id.bpid",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_BPID,
                NULL, HFILL}
            },
            {&tmsgf_id_nb,
                { "NoBeam", "ebrec.msg.id.nb",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_NB,
                NULL, HFILL}
            },
            {&tmsgf_id_va,
                { "VirtAcc", "ebrec.msg.id.va",
                FT_BOOLEAN, 64,
                NULL, TMSG_ID_VA,
                NULL, HFILL}
            },
            ...
            {&tmsgf_dl,
                { "Deadline", "timingmsg.msg.deadline",
                FT_UINT64, BA,
                NULL, 0x0,
                NULL, HFILL}
            },
...
*/
    ...
    proto_timingmsg = proto_register_protocol (
        "timingmsg Protocol", /* name       */
        "timingmsg",      /* short name */
        "tmsg"       /* abbrev     */
    );

    proto_register_field_array(proto_timingmsg, hf, array_length(hf));
    proto_register_field_array(proto_timingmsg, recf, array_length(recf));
